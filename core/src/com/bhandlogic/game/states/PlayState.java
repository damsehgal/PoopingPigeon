...
public class PlayState extends State
{
	...
	public PlayState(GameStateManager gsm)
	{
		...
		person = new Person(Person.PERSON_X_OFFSET, ground.getHeight() + GROUND_Y_OFFSET);
	...
	private void initializeWires()
	{
		...
		{
			wires1.add(new Taar(Khamba.KHAMBA_WIDTH + (i + 1) * (Taar.TAAR_SPACING + Taar.TAAR_WIDTH)));
	...
	private void updateKhambaTaarAndBird(float dt)
	{
		for (int i = 0; i < khambe.size; i++)
		{
			...
			{
				khamba.reposition(khamba.getPosition().x + ((Khamba.KHAMBA_WIDTH + KHAMBA_SPACING) * KHAMBA_COUNT));
			}
			Taar taar = wires1.get(i);
			if (camPosition > taar.getXOffsetWithWidth())
			{
				taar.reposition(taar.getPosition().x + ((Taar.TAAR_WIDTH + Taar.TAAR_SPACING) * KHAMBA_COUNT));
			}

			Taar taar2 = wires2.get(i);
			if (camPosition > taar2.getXOffsetWithWidth())
			{
				taar2.reposition(taar2.getPosition().x + ((Taar.TAAR_WIDTH + Taar.TAAR_SPACING) * KHAMBA_COUNT));
			}
			for (Taar t_taar : wires2)
			{
				...
				for (Bird mBird : bi)
				{
					mBird.update(dt, cam.position.x > mBird.getPosition().x);
				}
			}
		}
	}
	...
	public void render(SpriteBatch spriteBatch)
	{
		...
		{
			spriteBatch.draw(khamba.getTexture(), khamba.getPosition().x, khamba.getPosition().y, Khamba.KHAMBA_WIDTH, Khamba.KHAMBA_HEIGHT);
		}
		spriteBatch.draw(ground, groundPos1.x, groundPos1.y);
		spriteBatch.draw(ground, groundPos2.x, groundPos2.y);
		for (Taar taar : wires1)
		{
			spriteBatch.draw(taar.getTexture(), taar.getPosition().x, taar.getPosition().y, taar.TAAR_WIDTH, Taar.TAAR_HEIGHT);
		}
		for (Taar taar : wires2)
		{
			...
			for (Bird bird : pakshiArray)
			{
				...
				spriteBatch.draw(birdPoop.getTexture(), birdPoop.getPosition().x, birdPoop.getPosition().y, GroundPoop.SCALE_X * birdPoop.getTexture().getRegionWidth(), GroundPoop.SCALE_Y * birdPoop.getTexture().getRegionHeight());
			}
		}
		TextureRegion texture = person.getTexture();
		spriteBatch.draw(texture, person.getPosition().x, person.getPosition().y, texture.getRegionWidth(), texture.getRegionHeight());
		spriteBatch.end();
	}
	@Override
	public void dispose()
